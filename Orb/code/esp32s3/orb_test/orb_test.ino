...
// At 50Hz with 14-bit: 1 tick = 1.22us
// 500us  = ~410 ticks  (0 degrees)
// 1500us = ~1228 ticks (90 degrees)
// 2400us = ~1966 ticks (180 degrees)
...
void setup() {
    ...
    Serial.println();
    Serial.println("================================");
    Serial.println("   ORB TEST FIRMWARE v1.0");
    ...
    Serial.println("[GPS] Waiting for data... (may take 30-60s for fix outdoors)");
    ...
    if (mpu.testConnection()) {
        ...
        Serial.println("[IMU] Check: SDA->GPIO18, SCL->GPIO8, AD0->GND");
    }
    ...
    if (bmp.begin(0x76)) {
        ...
        Serial.println("[BARO] Check: CSB->3.3V for address 0x76");
...
void scanI2C() {
    ...
    if (count == 0) {
        ...
        Serial.println("  Check wiring: SDA=GPIO18, SCL=GPIO8");
    } else {
...
void servoSweepTest() {
    ...
    Serial.println("  Moving all servos: 90 -> 60 -> 120 -> 90");
...
void centerServos() {
    Serial.println("\n[SERVO] Centering all servos to 90 degrees...");
    servoWriteAll(90);
...
void showRawGpsData() {
    ...
    if (charCount == 0) {
        ...
        Serial.println("      Check: TX->GPIO17, RX->GPIO16, VCC->3.3V");
    }
    Serial.println();
}
