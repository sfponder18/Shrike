...
| Chick Commands | GCS → Chick | Meshtastic (direct) | On demand |
| Chick Telemetry | Chick → GCS | Meshtastic (direct) | 1-2Hz |
| Orb Target Upload | GCS → Chick → Orb | Mesh + Serial | On demand |
| Orb Arm/Release | GCS → Chick → Orb | Mesh + Serial | On demand |
| Emergency/Backup | Any → Any | Meshtastic | As needed |

**Note:** Orbs do not have mesh radios in V1. Target coordinates and arm/release commands are sent to Chicks via mesh, then Chicks upload to Orbs via serial connection before release.
...
NODE: Sender ID
  ...
  2 = Chick1
  3 = Chick2
  4-7 = Reserved (future: Orb radios)
...
────────────────────────────────────────────────────────────────
0x01    ARM_SYSTEM      0=disarm, 1=arm (vehicle motors)
0x02    RTL             -
0x03    LAND            -
0x04    LOITER          -
...
0x08    ARM_ORB         Orb slot (1-4), 0=disarm
0x09    RELEASE_ORB     Orb slot (1-4)
0x0A    TARGET_ORB      Orb slot in PARAM, followed by TGT msg
0x0B    SCAN_START      Band index
0x0C    SCAN_STOP       -
0x0D    REPORT          Request telemetry burst
...
EXAMPLE: Arm Orb slot 2 on Chick1
Encoded: 02 02 08 02

EXAMPLE: Disarm Orb slot 2 on Chick1 (PARAM=0 means disarm current)
Encoded: 02 02 08 00
...
Used to send target coordinates to Chicks for Orb upload.
GCS sends to Chick, Chick stores and uploads to Orb via serial.

BYTE    FIELD           ENCODING
────────────────────────────────────────────────────────────────
0       TYPE            0x04
1       TARGET_NODE     Recipient Chick (2=Chick1, 3=Chick2)
2-4     LAT             uint24 encoded (same as TELEM)
5-7     LON             uint24 encoded
8-9     ALT             Altitude in decimeters (target alt)
10      ORB_SLOT        Which orb slot (1 or 2)
11      FLAGS           0x01=immediate upload, 0x00=store only

WORKFLOW:
1. GCS sends TGT message to Chick with coordinates + orb slot
2. Chick stores target for specified orb slot
3. On ARM_ORB command, Chick uploads target to Orb via serial
4. On RELEASE_ORB command, Chick triggers release mechanism

EXAMPLE: Send target to Chick1 for Orb slot 1
Encoded: 04 02 8A3B2F 4D6E1A 0000 01 00
...
# Command codes
CMD_ARM_SYSTEM = 0x01   # Arm/disarm vehicle motors
CMD_RTL = 0x02
CMD_LAND = 0x03
CMD_LOITER = 0x04
CMD_RESUME = 0x05
CMD_HOLD = 0x06
CMD_MODE = 0x07
CMD_ARM_ORB = 0x08      # Arm orb (param=slot, 0=disarm)
CMD_RELEASE_ORB = 0x09  # Release orb (param=slot)
CMD_TARGET_ORB = 0x0A   # Target orb (followed by TGT msg)
CMD_SCAN_START = 0x0B
CMD_SCAN_STOP = 0x0C
CMD_REPORT = 0x0D
CMD_REBOOT = 0xFE
...
# Node IDs (V1: only GCS, Bird, Chicks have radios)
NODE_GCS = 0
NODE_BIRD = 1
NODE_CHICK1 = 2
NODE_CHICK2 = 3
# NODE_ORB1-4 = 4-7 reserved for future Orb radios
NODE_BROADCAST = 0xFF
...
CODEWORD TYPES:
  ...
  0x02 CMD      4B  Command code
  0x03 ACK      3B  Acknowledgment
  0x04 TGT     12B  Target coordinates (to Chicks)
  0x07 SDR     20B  SDR summary

COMMAND CODES:
  0x01 ARM_SYSTEM   Arm/disarm vehicle
  0x08 ARM_ORB      Arm orb slot (0=disarm)
  0x09 RELEASE_ORB  Release orb slot
  0x0A TARGET_ORB   Send target to orb
...
| 2.1 | Jan 2026 | Clarified Orb data flow (Chick→Orb via serial), separated ARM_SYSTEM/ARM_ORB commands, updated node topology |
