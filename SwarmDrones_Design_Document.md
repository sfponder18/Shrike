...
═══════════════════════════════════════════════════════════════
     ...
     BIRD (Ranger)                    CHICK x2 (Baby Ape II)
    ┌─────────────┐                  ┌─────────────┐
...
| Component | Specification | Source | Cost |
|-----------|---------------|--------|------|
| Raspberry Pi 5 8GB | Single board computer | PiShop/Amazon | $80 |
| MicroSD Card | 64GB Class 10 | Amazon | $12 |
| Active Cooler | Official Pi 5 cooler | PiShop | $5 |
| 5V BEC | UBEC 5V 3A | Amazon | $8 |

*Note: HackRF moved to GCS for v1. Bird SDR and AI capabilities deferred to v2 with Jetson upgrade (see Future_Capabilities.md)*

**Raspberry Pi 5 Specs:**
- Quad-core Arm Cortex-A76 @ 2.4GHz
- 8GB LPDDR4X
- USB 3.0, PCIe, dual HDMI
- Power: 5V 5A (25W max)
- Sufficient for: MAVLink, video streaming, mesh bridge
...
| MLRS RX | E77 868MHz module | AliExpress | $25 |
| T-Beam | Lilygo T-Beam 868MHz | AliExpress | $25 |
...
|-----------|---------------|--------|-----------|
| BNF Quad | Darwin FPV Baby Ape II | Darwin FPV | $170 |
| ELRS RX | RadioMaster RP2 | GetFPV | $18 |

**Baby Ape II Includes:**
- Frame: 3" prop guard frame
- Motors: 1404 motors
- ESC: 4-in-1 ESC
- FC: F4/F7 (ArduCopter compatible)
- VTX: Included
- Camera: Included analog

*Note: Using ArduCopter firmware (not Betaflight) for MAVLink integration*

### Companion Computer + SDR + Comms
...
| Component | Specification | Source | Cost Each |
|-----------|---------------|--------|-----------|
| Flight Battery | 850mAh 4S x3 (3" size) | Amazon | $60 |
...
| Component | Specification | Source | Cost |
|-----------|---------------|--------|------|
| MLRS TX Module | E77 868MHz JR bay/USB | AliExpress | $35 |
| T-Beam | Lilygo T-Beam 868MHz | AliExpress | $25 |
...
├── USB-A ──► 4G Modem
├── USB-A ──► T-Beam (Meshtastic)
├── UART ───► Pixhawk TELEM1 (GPIO 14/15)
...
The Darwin FPV Baby Ape II comes ready to fly. Modifications needed:
...
### Step 2: Flash ArduCopter

Flash ArduCopter firmware for MAVLink integration:
- Unlocks GPS modes, waypoints
- MAVLink native (required for MLRS telemetry)
- Integrates with QGroundControl

### Step 3: Mount Pi Zero 2W + RTL-SDR + T-Beam
...
Weight Addition: ~95g (Pi + SDR + T-Beam + mount)
Note: Baby Ape II is a 3" quad, payload capacity is limited.
Consider external mounting if weight is an issue.
...
═══════════════════════════════════════════════════════════════
    ...
    │                                                         │
    │  ┌─────────────────┐  ┌─────────────────────────────┐  │
    │  │ QGroundControl  │  │    Custom Dashboard         │  │
    │  │                 │  │  ┌───────────────────────┐  │  │
    │  │  - Bird map     │  │  │ Mesh status, Orb mgmt │  │  │
    │  │  - Chick map    │  │  │ Target queue, Commands│  │  │
    │  │  - Telemetry    │  │  └───────────────────────┘  │  │
    │  │  - Mission      │  │                             │  │
    │  └─────────────────┘  └─────────────────────────────┘  │
    │                                                         │
    │  USB Connections:                                       │
    │  ├── MLRS TX Module ──► MAVLink telemetry              │
    │  ├── T-Beam ──► LoRa commands                          │
    │  ├── HackRF ──► Future EW (see Future_Capabilities.md) │
    │  └── Phone Hotspot ──► Internet (4G backup via Tailscale)│
    │                                                         │
    └─────────────────────────────────────────────────────────┘
...
See `docs/GCS_Context.md` for detailed GCS design.
...
| Custom Dashboard | Mesh status, Orb management | Python/PyQt5 (TBD) |
| Tailscale | VPN for 4G backup | tailscale.com |
| PuTTY | SSH to Pi | putty.org |
| VLC | Video streaming | videolan.org |

## 6.3 Connection Setup

### MLRS (Primary Telemetry)
```
QGroundControl:
  Comm Links → Add
  Type: Serial
  Port: COMx (MLRS TX module)
  Baud: 115200
```

### 4G Connection (Backup via Tailscale)
```
Setup:
  - Install Tailscale on Pi 5 and GCS laptop
  - Both join same Tailscale network
  - Pi 5 exposes MAVLink on UDP 14550

QGroundControl:
  Comm Links → Add
  Type: UDP
  Port: 14550
  Server: (Pi 5 Tailscale IP, e.g., 100.x.x.x)
```
...
Link Summary:
- Bird Primary: MLRS 868MHz (MAVLink telemetry)
- Bird Backup: 4G via Tailscale
- Bird RC: ELRS 2.4GHz
- LoRa Commands: T-Beam 868MHz mesh (all nodes)
- Chick Telemetry: MLRS 868MHz
- Chick Data: WiFi via Bird → 4G
- Chick RC: ELRS 2.4GHz
...
| Data | Path | Rate |
|------|------|------|
| Bird telemetry | FC → MLRS → GCS (primary) | 10 Hz |
| Bird telemetry backup | FC → Pi 5 → 4G/Tailscale → GCS | 10 Hz |
| Bird video | Camera → Pi 5 → 4G → GCS | 720p 30fps |
| Chick telemetry | FC → MLRS → GCS | 10 Hz |
| Chick SDR | RTL-SDR → Pi → WiFi → Bird → 4G → GCS | Samples |
| LoRa commands | GCS T-Beam ↔ Vehicle T-Beams | On demand |
| MAVLink commands | GCS → MLRS → FC | On demand |
...
| Raspberry Pi 5 8GB | PiShop/Amazon | 3-5 days |
...
| 3.1 | Jan 2026 | Comms update: MLRS+T-Beam, Baby Ape II, ArduCopter, HackRF to GCS |

**Related Documents:**
- `docs/GCS_Context.md` - Detailed GCS design
- `docs/Future_Capabilities.md` - Deferred features
- `docs/COMPANION_COMPUTER_FEATURES.md` - CC feature spec (needs update)
